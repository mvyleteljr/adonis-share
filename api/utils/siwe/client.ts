...
export class SiweMessage {
   ...
   */
  async validate(provider?: ethers.providers.Provider | any): Promise<SiweMessage> {
    ...
    return new Promise<SiweMessage>(async (resolve, reject) => {
      ...
      try {
        ...
        const addr = ethers.utils.verifyMessage(message, this.signature)

        if (addr !== this.address) {
          // eslint-disable-next-line
          try {
            //EIP-1271
            const isValidSignature = await checkContractWalletSignature(this, provider)
            if (!isValidSignature) {
...
export const checkContractWalletSignature = async (
  ...
  try {
    ...
    return await walletContract.isValidSignature(hashMessage, message.signature)
  } catch (e) {
    throw e
  }
}
